...
- Run the `wait-and-result` call in the FOREGROUND (a normal blocking `Bash` call). It blocks internally and prints the full result to stdout when finished; that IS your answer. Give the `Bash` call a long timeout (use the maximum you can, e.g. `timeout: 600000`) so it isn't cut off early. NEVER run it with `run_in_background`, and NEVER reply with "I'll wait for the result", "I'll check back", or similar — that abandons the run without returning anything. Make the one blocking call and return its stdout.
- Recovery: the command prints a line like `[opencode] job <id> dispatched…` to stderr at the start. If your blocking `Bash` call is nonetheless cut off (its own timeout) before the result prints, do NOT give up — the detached worker keeps running and finishes server-side. Make ONE more `Bash` call, `node "${CLAUDE_PLUGIN_ROOT}/scripts/opencode-companion.mjs" result <id>` (using the id from that stderr line), to fetch and return the completed result.
